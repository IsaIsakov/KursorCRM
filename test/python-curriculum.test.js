...
test('Python curriculum provides two ordered, substantial tracks', () => {
  ...
  assert.equal(lessons.reduce((sum, x) => sum + x.tasks.length, 0), 160);
  ...
  for (const lesson of lessons) {
    ...
    assert.equal(lesson.tasks.length, 10, lesson.moduleId);
    assert.ok(lesson.tasks.filter(t => t.type === 'code').length >= 4, lesson.moduleId);
    assert.ok(lesson.tasks.filter(t => t.type === 'quiz').length >= 3, lesson.moduleId);
    assert.equal(new Set(lesson.tasks.map(t => t.title)).size, 10, lesson.moduleId);
