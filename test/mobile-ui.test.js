...
test('every user-facing page opts into safe-area mobile layout and versioned CSS', () => {
  for (const file of pages) {
    ...
    assert.match(html, /\/css\/style\.css\?v=29/, file);
  }
});
...
test('mobile navigation and admin drawer remain present in release assets', () => {
  ...
  assert.match(css, /\.modal\.lesson-modal\s*\{[\s\S]*max-width:1480px/);
  assert.match(css, /\.modal\.student-overview-modal\s*\{[\s\S]*max-width:1180px/);
  assert.match(css, /@media \(max-width:520px\)[\s\S]*\.modal\.lesson-modal[\s\S]*height:100dvh/);
});
